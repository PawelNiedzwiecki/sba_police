...
function Wdrozenie(props){
  ...
  useEffect(() =>{
    ...
    const wdrozenieID = props.match.params.wdrozenieID;
  ...
  function addNewComponent(){
    ...
    const template = {id: '', nazwaKomponentu: '', dgID:'', poczatkowyCzas: '', transformacja:''}
    ...
    return(
      <div>
        ...
        <table className="table table-hover">
          ...
          <tbody>
          {components.map(item => 
            <Komponent key={item.id} id={item.dgID} nazwa={item.nazwaKomponentu} poczatkowyCzas={item.poczatkowyCzas} transformacja={item.transformacja}/>
          )}
          </tbody>
        </table>
      </div>
...
function Komponent(props){
  ...
  return(
    <tr data_id={id}>
      ...
      <td><input className="componentName" value={nazwaKomponentu} onChange={(e) => handleChange(e.target)}></input></td>
